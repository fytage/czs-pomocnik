...
export default {
    ...
    async execute(interaction) {
        ...
        const [suggestionId, suggestionData] = suggestionEntry;
        
        const modal = new ModalBuilder()
            .setCustomId(`decision-modal_approve_${targetMessage.id}_${suggestionId}`)
            .setTitle(`Schválit Nápad #${suggestionId}`);

        const reasonInput = new TextInputBuilder()
            .setCustomId('decision-reason')
            .setLabel("Komentář k schválení")
        	.setMaxLength(1000)
    ...
    async handleModal(interaction) {
        try {
            ...
            const updatedEmbed = EmbedBuilder.from(originalEmbed)
                .setTitle(`Nápad #${suggestionId} Schválen`)
                .setColor(0xCAFFBF)
           ...
           const DMdecisionEmbed = new EmbedBuilder()
                .setTitle(`Tvůj Nápad #${suggestionId} byl Schválen`)
                .setColor(0xCAFFBF)
                .setAuthor({ name: moderator.displayName, iconURL: moderator.user.displayAvatarURL() })
                .setFields(originalEmbed.fields)
            ...
            if (logChannel) {
                const logEmbed = new EmbedBuilder()
                    .setTitle(`Nápad Schválen (#${suggestionId})`)
                    .setColor(0xCAFFBF)
                    .addFields(
            ...
            await interaction.editReply({ content: 'Nápad byl úspěšně schválen a diskuze uzamčena.', ephemeral: true });
            
        } catch(error) {
            console.error("Error handling approve modal:", error);
            await interaction.editReply({ content: 'Došlo k chybě při schvalování nápadu.', ephemeral: true });
        }
    }
};